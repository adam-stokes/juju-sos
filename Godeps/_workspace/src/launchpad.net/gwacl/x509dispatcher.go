...
// rewindCloser returns an io.ReadCloser that seeks back
// to the beginning of the reader upon Close being called.
type rewindCloser struct {
    io.ReadSeeker
}

func (c rewindCloser) Close() error {
    _, err := c.Seek(0, 0)
    return err
}
...
func performX509Request(session *x509Session, request *X509Request) (*x509Response, error) {
    ...
    bodyReader := rewindCloser{bytes.NewReader(request.Payload)}
    httpRequest, err := http.NewRequest(request.Method, request.URL, bodyReader)
    if err != nil {
        return nil, err
    }
    httpRequest.ContentLength = int64(len(request.Payload))
