...
func assertDeleteHostedServiceRequest(c *C, api *ManagementAPI, serviceName string, httpRequest *X509Request) {
    expectedURL := fmt.Sprintf("%s%s/services/hostedservices/%s?comp=media", defaultManagement,
        api.session.subscriptionId, serviceName)
    checkRequest(c, httpRequest, expectedURL, "2013-08-01", nil, "DELETE")
}
...
func assertDeleteRoleRequest(c *C, api *ManagementAPI, httpRequest *X509Request, serviceName, deploymentName, roleName string, deleteMedia bool) {
    expectedURL := (defaultManagement + api.session.subscriptionId +
        "/services/hostedservices/" +
        serviceName + "/deployments/" + deploymentName + "/roles/" + roleName)
    if deleteMedia {
    ...
    checkRequest(
        c, httpRequest, expectedURL, "2013-08-01", nil, "DELETE")
}

func (suite *managementBaseAPISuite) TestDeleteRole(c *C) {
    suite.testDeleteRole(c, false)
}

func (suite *managementBaseAPISuite) TestDeleteRoleDeleteMedia(c *C) {
    suite.testDeleteRole(c, true)
}

func (suite *managementBaseAPISuite) testDeleteRole(c *C, deleteMedia bool) {
    api := makeAPI(c)
    ...
    err := api.DeleteRole(request)
    c.Assert(err, IsNil)
    assertDeleteRoleRequest(
        c, api, recordedRequests[0], request.ServiceName,
        request.DeploymentName, request.RoleName, deleteMedia)
}

func assertAddRoleRequest(c *C, api *ManagementAPI, httpRequest *X509Request, serviceName, deploymentName, expectedXML string) {
    expectedURL := (defaultManagement + api.session.subscriptionId +
        "/services/hostedservices/" +
        serviceName + "/deployments/" + deploymentName + "/roles")
    checkRequest(
        c, httpRequest, expectedURL, "2013-10-01", []byte(expectedXML), "POST")
    c.Assert(httpRequest.ContentType, Equals, "application/xml")
}

func (suite *managementBaseAPISuite) TestAddRole(c *C) {
    api := makeAPI(c)
    ...
    err := api.AddRole(request)
    c.Assert(err, IsNil)

    expectedXML, err := request.PersistentVMRole.Serialize()
    c.Assert(err, IsNil)
    assertAddRoleRequest(
        c, api, recordedRequests[0], request.ServiceName,
        request.DeploymentName, expectedXML)
}
